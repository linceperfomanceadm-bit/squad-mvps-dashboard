      ...
      <div style={MS.modal} onClick={e => e.stopPropagation()} className="fade-up">
        ...
        <form onSubmit={handleSubmit} style={MS.body}>
          ...
          <div style={MS.field}>
            ...
            <select style={MS.input} value={form.wdService} onChange={e => set('wdService', e.target.value)}>
            ...
            {Object.values(SECTORS).map(s => {
              ...
              return (
                <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                  <span style={{ fontSize: 13, color: s.color, minWidth: 100, display: 'flex', alignItems: 'center', gap: 5 }}>{s.emoji} {s.label}</span>
                  <select style={{ ...MS.input, flex: 1 }} value={form.responsibles[s.id] || ''} onChange={e => set('responsibles', { ...form.responsibles, [s.id]: e.target.value })}>
...
export default function AdminClients({ clients, collaborators, onAdd, onUpdate, onDelete, onWdMoveToProduction, onWdMoveBackToOnboarding, onWdMoveStatus }) {
  ...
  const wdStatusLabel = (status) => {
    const map = { onboarding: 'Onboarding', production: 'Produção', inactive: 'Inativo', recurrence: 'Recorrência', finished: 'Finalizado' };
    return map[status] || status;
  };
  const wdStatusColor = (status) => {
    const map = { onboarding: 'var(--blue)', production: 'var(--neon)', inactive: 'var(--muted)', recurrence: 'var(--purple)', finished: 'var(--green)' };
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)', background: 'rgba(255,255,255,.02)' }}>
                {['Cliente', 'WebDesign', 'Serviço WD', ...Object.values(SECTORS).map(s => s.label), 'Ações'].map(h => (
            ...
            <tbody>
              {filtered.map(c => (
                <tr key={c.id} style={{ borderBottom: '1px solid rgba(255,255,255,.04)' }}>
                  ...
                  {Object.values(SECTORS).map(s => (
                    <td key={s.id} style={{ padding: '12px 14px', fontSize: 12, color: 'var(--muted)', whiteSpace: 'nowrap' }}>
                      {c.responsibles?.[s.id] || '—'}
                    </td>
                  ))}
                  <td style={{ padding: '12px 14px' }}>

...
export default function AdminOverview({ clients, collaborators, onNavigate }) {
  ...
  // Design + Video deliveries this month
  const allDeliveries = [];
  clients.forEach(c => {
  ...
  // SM stuck in client column > 3 days
    ...
    <div className="fade-up">
      ...
      <div style={{ background: 'rgba(12,12,24,.88)', border: '1px solid var(--border)', borderRadius: 14, padding: '20px 22px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(220px,1fr))', gap: 12 }}>
          {sectorData.map(s => (
            <div key={s.id} style={{ background: `${s.color}08`, border: `1px solid ${s.color}25`, borderRadius: 12, padding: '16px' }}>

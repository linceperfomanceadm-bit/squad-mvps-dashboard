...
export default function HallOfFame({ tasks }) {
  // Build ranking from completed tasks with no rework (approved first time)
  const firstApprovalTasks = tasks.filter(
    t => t.status === 'done' && t.reworkCount === 0
  );

  // Count by collaborator (requestedBy is the sector, we rank by responsible who delivered)
  const rankMap = {};
  firstApprovalTasks.forEach(t => {
    const key = t.responsibleName || 'Desconhecido';
  ...
  // Recent first-approval deliveries (for the grid)
  ...
  return (
    <div className="fade-up">
      ...
      <div style={{ background: 'rgba(12,12,24,.88)', border: '1px solid var(--border)', borderRadius: 14, padding: '22px', marginBottom: 20 }}>
        <h2 style={{ fontSize: 15, fontWeight: 700, color: 'var(--text)', marginBottom: 18 }}>
          Ranking Geral
        </h2>
        ...
        {ranking.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {ranking.map((collab, idx) => {
              ...
              const borderColors = { 1: 'rgba(255,215,0,0.2)', 2: 'rgba(192,192,192,0.2)', 3: 'rgba(205,127,50,0.2)' };
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    background: isTop3 ? bgColors[rank] : 'var(--surface)',
                    border: `1px solid ${isTop3 ? borderColors[rank] : 'var(--border)'}`,
                ...
                >
                  ...
                  <div style={{
                    ...
                    background: 'linear-gradient(135deg,var(--neon-dim),rgba(167,139,250,.1))',
                    border: '1px solid var(--neon-border)',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontSize: 16, fontWeight: 800, color: 'var(--neon)', flexShrink: 0,
                  }}>
        ...
        <div style={{ background: 'rgba(12,12,24,.88)', border: '1px solid var(--border)', borderRadius: 14, padding: '22px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(220px,1fr))', gap: 10 }}>
            {recentFirstApproval.map(task => (
              ...
              >
                <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', marginBottom: 3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {task.name}
                </p>
                ...
                {task.links?.length > 0 && (
                  <a
                    href={task.links[0]} target="_blank" rel="noreferrer"
                    style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, color: 'var(--green)', textDecoration: 'none' }}
                  >

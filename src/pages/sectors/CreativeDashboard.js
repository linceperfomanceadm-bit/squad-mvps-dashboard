...
export default function CreativeDashboard({ sectorId }) {
  ...
  const myDeliveries = [];
  clients.forEach(c => {
    const deliveries = sectorId === 'design' ? (c.design?.deliveries || []) : (c.video?.deliveries || []);
    deliveries.forEach(d => {
      if (d.responsible === user?.name) myDeliveries.push({ ...d, clientName: c.name, clientId: c.id });
    });
  });

  const handleAddDelivery = async (data) => {
    const res = await addDelivery(data.clientId, deliveryKey, { ...data, responsible: user?.name });
    if (res.success) { toast('Entrega cadastrada!'); setShowDeliveryModal(false); }
    ...
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      ...
      <main style={{ flex: 1, marginLeft: 224, padding: 32, minHeight: '100vh', overflow: 'auto' }}>
        ...
        <button
          ...
          style={{ position: 'fixed', bottom: 28, right: 28, zIndex: 100, background: `linear-gradient(135deg,${sectorId === 'design' ? '#a78bfa,#7c3aed' : '#fb923c,#ea580c'})`, border: 'none', borderRadius: 14, padding: '14px 22px', color: '#fff', fontSize: 14, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 8, boxShadow: '0 8px 28px rgba(0,0,0,.4)', cursor: 'pointer' }}
        >

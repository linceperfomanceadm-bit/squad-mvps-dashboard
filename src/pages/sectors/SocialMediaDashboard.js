...
import { LayoutDashboard, Columns, Plus } from 'lucide-react';
...
export default function SocialMediaDashboard() {
  ...
  // Only clients where this collaborator is responsible for Social Media
  const myClients = clients.filter(
    c => c.active && c.responsibles?.socialmedia === user?.name
  );

  // Only this collaborator's posts (from their clients)
  const myPosts = [];
  myClients.forEach(c => {
    (c.sm?.posts || []).forEach(p => {
      if (p.responsible === user?.name) {
        myPosts.push({ ...p, clientName: c.name, clientId: c.id });
      }
    });
  });
  ...
  const handleBulkSave = async (rows) => {
    ...
    if (res.success) {
      toast(`${rows.length} post${rows.length > 1 ? 's' : ''} adicionado${rows.length > 1 ? 's' : ''} ao Kanban!`);
      setPage('kanban');
    } else {
      toast(res.error, 'e');
    }

...
Frontend deploys to Vercel (`vercel.json`: SPA rewrite of everything to `/`, output `build/`). Only the six `REACT_APP_FIREBASE_*` vars are actually required — without them `initializeApp` receives an undefined config and the app renders blank. `REACT_APP_ADMIN_ID` and `REACT_APP_ADMIN_PASSWORD` are optional only in the sense that the code carries a hardcoded fallback pair when they are unset (`AuthContext.js:34-35`) — always set them explicitly. `functions/` deploys separately through the Firebase CLI — it is not part of the Vercel build.

**That fallback grants real admin access, not a stub.** Matching it sends the login through `tryLazyMigration` (`:182`), which creates an actual Firebase Auth account; `loadProfileByUid` (`:95-97`) then returns a synthetic `superadmin` profile and writes `userIndex/{uid}.isAdmin = true` — the very record the `resetCollaboratorPassword` Cloud Function trusts to authorize resetting any collaborator's password. Compounding it, every `REACT_APP_*` value is compiled into the client bundle, so whichever admin password is configured is shipped to the browser regardless. Rotating the literal does not fix this; the fix is to validate the admin **outside the client** — custom claims or a callable function — so no credential decides admin rights in front-end code.
